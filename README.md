...
- **In-Place Memory Modification:** To bypass strict mobile memory constraints, Prism fundamentally avoids representing model weights as dynamic input tensors. Instead, the system directly reads the model file into a large `std::vector<uint8_t>` buffer and applies a direct, in-place memory modification strategy using raw pointers. 
- **High-Throughput On-Device Sampling:** Standard random number generators are replaced with mobile-optimized `Xoshiro256++` and the Ziggurat algorithm, providing the extreme sampling throughput required by the SDS module.
...
- **`server/`**: The Python-based training framework and benchmarking suite. It contains scripts for evaluating various architectures (e.g., LLaMA, Phi, Qwen, TinyLlama), comparing First-Order (FO) and ZO optimization, and simulating memory occupancy. For setup and execution details, refer to [`server/README.md`](./server/README.md).
