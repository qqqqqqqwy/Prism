...
struct StepTimes {
    ...
    void print(int step) const {
        printf(
            ...
            "      +perturb (gen+apply): %.3f ms | +clear: %.3f ms | +decode: %.3f ms\n"
            "      -perturb (gen+apply): %.3f ms | -clear: %.3f ms | -decode: %.3f ms\n"
            "      update   (gen+apply): %.3f ms\n",
...
void apply_perturbation_in_place_parallel(const std::vector<LayerInfo>& layers, uint64_t step_seed, float scale, float std_dev) {
        ...
        workers.emplace_back([&]() {
            while (true) {
                ...
                for (int64_t j = 0; j < count; ++j) {
                    ...
                    float val = fp16_to_fp32(data_ptr[j]);
                    val += scale * noise;
...
int main(int argc, char ** argv) {
    ...
    for (int step = 0; step < 5; ++step) {
        ...
        tm.t_perturb_pos_ms = ms_since(t_start, Clock::now());
        ...
        tm.t_perturb_neg_ms = ms_since(t_start, Clock::now());
        ...
        tm.t_update_ms = ms_since(t_start, Clock::now());
