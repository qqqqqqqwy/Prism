...
for i in "${!TESTS[@]}"; do
    ...
    # 记录前快照
    adb -s 192.168.137.127 shell dumpsys batterystats > "${PREFIX}-before.txt"
    
    # 执行程序
    echo " -> 正在运行 $CMD ..."
    adb -s 192.168.137.127 shell "cd /data/local/tmp/llama-zoo && LD_LIBRARY_PATH=\$PWD:\$LD_LIBRARY_PATH $CMD"
    
    # 记录后快照
    adb -s 192.168.137.127 shell dumpsys batterystats > "${PREFIX}-after.txt"
