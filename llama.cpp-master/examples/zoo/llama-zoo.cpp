...
// Helper: tokenize a string using llama.cpp API
static std::vector<llama_token> tokenize_text(
    ...
    // n_tokens is negative => abs(n_tokens) = required size
    if (n_tokens < 0) {
        n_tokens = -n_tokens;
    }
...
// Helper: convert a token to its string representation
...
// SST-2 verbalizer: 0 -> "terrible", 1 -> "great"
static const char * sst2_verbalizer[] = {"terrible", "great"};

// Load SST-2 from TSV file
// Format: sentence\tlabel  (first line is header)
static std::vector<Sample> load_sst2_tsv(
    const char * path,
    const struct llama_vocab * vocab,
    int max_samples,
    int max_length,
    std::vector<llama_token> & cls_idx_out  // collect unique candidate tokens
) {
    ...
    while (std::getline(fin, line) && (int)samples.size() < max_samples) {
        ...
        // Verbalize: "{sentence} It was {verbalizer[label]}"
        std::string verbalized = sentence + " It was " + sst2_verbalizer[label];

        // Tokenize full sentence (with BOS)
        std::vector<llama_token> input_ids = tokenize_text(vocab, verbalized, true);
        ...
        llama_token cand_token = input_ids.back();
        if (std::find(cls_idx_out.begin(), cls_idx_out.end(), cand_token) == cls_idx_out.end()) {
            cls_idx_out.push_back(cand_token);
        }

        Sample s;
        s.input_ids = input_ids;
        s.label = label;
        s.target_token = cand_token;
        samples.push_back(s);
    }
...
static float evaluate(
    ...
    // Process eval samples one at a time (simple, no batching for eval)
    for (size_t si = 0; si < eval_samples.size(); ++si) {
        ...
        for (int i = 0; i < n_tokens; ++i) {
            ...
            batch.n_seq_id[i] = 1;
            batch.seq_id[i][0]= 0;
            batch.logits[i]   = (i == n_tokens - 2) ? 1 : 0; 
        }
        ...
        float * logits = llama_get_logits_ith(ctx, n_tokens - 2);
        if (!logits) {
            llama_batch_free(batch);
            continue;
        }

        // Find which cls_idx token has the highest logit
        int best_cls = -1;
        ...
        // cls_idx is ordered: cls_idx[0] = token for label 0 ("terrible"),
        //                      cls_idx[1] = token for label 1 ("great")
...
int main(int argc, char ** argv) {
    ...
    // Context size: max_length * batch_size  (worst case)
    ...
    std::mt19937 data_rng(42);  // for shuffling data indices
    ...
    for (int step = 0; step < n_steps; ++step) {
        ...
        // Build the batch (pad shorter sequences with EOS/pad token)
        ...
        // Track where each sample's last token is (for loss computation)
        ...
        for (int b = 0; b < batch_size; ++b) {
            ...
            for (int i = 0; i < max_seq_len; ++i) {
                ...
                batch.logits[token_idx] = (i == seq_len - 2) ? 1 : 0;
                if (i == seq_len - 2) {
        ...
        apply_perturbation_in_place_parallel(target_layers, step_seed, 1.0f, sigma,
        ...
        for (int b = 0; b < batch_size; ++b) {
            ...
            if (logits) {
                ...
                float target_logit = logits[sample_target_tokens[b]];
                loss_pos += -target_logit + max_logit + std::log(sum_exp);
            }
        }
        loss_pos /= batch_size;
        ...
        for (int b = 0; b < batch_size; ++b) {
            ...
            if (logits) {
                ...
                float target_logit = logits[sample_target_tokens[b]];
                loss_neg += -target_logit + max_logit + std::log(sum_exp);
            }
        }
        loss_neg /= batch_size;
        ...
        t_start = Clock::now();
        apply_perturbation_in_place_parallel(target_layers, step_seed, update_scale, sigma,
