...
void generate_and_save_noise_to_disk_parallel(const std::vector<LayerInfo>& layers, float std_dev) {
    std::atomic<size_t> layer_idx(0); // 原子计数器，用于分发任务
...
void apply_noise_scale_parallel(std::vector<LayerInfo>& layers, float scale) {
...
int main(int argc, char ** argv) {
    ...
    for (int step = 0; step < 5; ++step) {
        ...
        generate_and_save_noise_to_disk_parallel(target_layers, sigma);
        t_temp_end = Clock::now();
        tm.t_noise_gen_ms = ms_since(t_temp_start, t_temp_end);

        t_temp_start = Clock::now();
        apply_noise_scale_parallel(target_layers, 1.0f);
        t_temp_end = Clock::now();
        ...
        t_temp_start = Clock::now();
        apply_noise_scale_parallel(target_layers, -2.0f);
        t_temp_end = Clock::now();
        ...
        apply_noise_scale_parallel(target_layers, restore_and_update_scale);
        t_temp_end = Clock::now();
